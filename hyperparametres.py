...
# -*- coding: utf-8 -*-
"""
Created on Mon Jan 13 18:27:05 2020
...
for i in range(10): # On fait 10 epoch
    ...
    while (time.time()-temps_initial < temps_max): 
        ...
        population = selection.selection_population_p_meilleurs(population, p) # Sélection
...
for i in range(10): # On fait 10 epoch
    ...
    while (time.time()-temps_initial < temps_max): 
        population = croisement.croisement_population(population) # Croisement
        mutation.mutation_population(population, 10) # Mutation
        population = selection.selection_population_roulette(population) # Sélection
        if C > population[0].dur: # Sauvegarde du meilleur individu
            C = population[0].dur
            meilleure_sequence = population[0].seq
...
for i in range(10): # On fait 10 epoch
    ...
    while (time.time()-temps_initial < temps_max): 
        population = croisement.croisement_population(population) # Croisement
        mutation.mutation_population(population, 10) # Mutation
        population = selection.selection_population_tournoi(population, T=10) # Sélection
        if C > population[0].dur: # Sauvegarde du meilleur individu
            C = population[0].dur
            meilleure_sequence = population[0].seq
